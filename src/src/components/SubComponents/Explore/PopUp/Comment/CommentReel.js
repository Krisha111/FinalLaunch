...
export default function CommentReel({ reel, reelId, onCommentSend }) {
  ...
  const createdAt = new Date(reel?.createdAt).toLocaleString();
  ...
  const handleSendComment = async () => {
    ...
    onCommentSend({ reelId, text: payload });
    setCurrentComment('');
  ...
  return (
    ...
    >
      <View style={styles.topContainer}>
        {reel?.reelScript && <Text style={styles.reelScript}>{reel.reelScript}</Text>}
...
const styles = StyleSheet.create({
  ...
  reelScript: { fontSize: 16, fontWeight: 'bold', marginBottom: 5 },
